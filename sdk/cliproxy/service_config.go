...
// ValidateConfig validates a complete runtime configuration before application.
func (s *Service) ValidateConfig(_ context.Context, cfg *config.Config) error {
	...
	return cfg.ValidateCredentialWeights()
}
...
func (s *Service) ApplyConfig(ctx context.Context, cfg *config.Config) error {
	if errValidate := s.ValidateConfig(ctx, cfg); errValidate != nil {
		return errValidate
	}
	if !s.applyConfigUpdateWithAuthSynthesis(ctx, cfg, true) {
		if ctx != nil && ctx.Err() != nil {
			return ctx.Err()
		}
		return errors.New("configuration runtime apply did not complete")
	}
	return nil
}
