...
func TestRegisterModelsForAuthWithCacheAppliesAutomaticThinkingAliases(t *testing.T) {
	service := &Service{cfg: &config.Config{
		OpenAICompatibility: []config.OpenAICompatibility{{
			Name: "mimo",
			Models: []config.OpenAICompatibilityModel{{
				Name:     "upstream-reasoning",
				Alias:    "mimo-reasoning",
				Thinking: &registry.ThinkingSupport{Levels: []string{"high"}},
			}},
		}},
	}}
	auth := &coreauth.Auth{
		ID:       "compat-cache-thinking-alias",
	...
	service.registerModelsForAuthWithCache(
		context.Background(),
		auth,
		service.newOpenAICompatibilityRegistrationCache(),
	)

	if !GlobalModelRegistry().ClientSupportsModel(auth.ID, "mimo-reasoning-high") {
		t.Fatal("cached compatibility registration omitted automatic thinking alias")
	}
}
