...
import (
	...
	internalthinking "github.com/router-for-me/CLIProxyAPI/v7/internal/thinking"
...
func (s *Service) registerModelsForAuthWithCache(ctx context.Context, a *coreauth.Auth, compatCache *openAICompatibilityRegistrationCache) {
		...
		if s.cfg != nil {
			...
			registerCompat := func(compat *config.OpenAICompatibility) bool {
				...
				if len(ms) > 0 {
					...
					s.registerResolvedModelsForAuth(a, providerKey, applyAutomaticThinkingAliases(applyModelPrefixes(ms, a.Prefix, s.cfg.ForceModelPrefix), nil))
				} else {
			...
			if isCompatAuth {
				...
				if len(models) > 0 {
					s.registerResolvedModelsForAuth(a, providerKey, applyAutomaticThinkingAliases(applyModelPrefixes(models, a.Prefix, s.cfg != nil && s.cfg.ForceModelPrefix), nil))
				} else {
	...
	if len(models) > 0 {
		registrationModels := applyModelPrefixes(models, a.Prefix, s.cfg != nil && s.cfg.ForceModelPrefix)
		if authKind != "apikey" {
			registrationModels = applyAutomaticThinkingAliases(registrationModels, excluded)
		}
		s.registerResolvedModelsForAuth(a, key, registrationModels)
		return
	}
...
func buildOpenAICompatibilityConfigModels(compat *config.OpenAICompatibility) []*ModelInfo {
	...
	for i := range compat.Models {
		...
		payloads := internalthinking.NormalizeThinkingPayloads(model.ThinkingPayloads)
		thinkingSupport := model.Thinking
		if thinkingSupport == nil && len(payloads) > 0 {
			if levels := internalthinking.PayloadLevelKeys(payloads); len(levels) > 0 {
				thinkingSupport = &registry.ThinkingSupport{Levels: levels}
			}
		}
		...
		info.ThinkingPayloads = payloads
...
func applyOAuthModelAliasEntries(aliases []config.OAuthModelAlias, models []*ModelInfo) []*ModelInfo {
	...
	for i := range aliases {
		...
		nameResult := internalthinking.ParseSuffixAllowHyphen(name)
