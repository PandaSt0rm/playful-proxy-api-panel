...
func TestGeminiExecutorNativeInteractionsResolvesRegisteredThinkingAlias(t *testing.T) {
	...
	const clientID = "gemini-interactions-thinking-alias"
	reg := registry.GetGlobalRegistry()
	reg.RegisterClient(clientID, "gemini-interactions", []*registry.ModelInfo{{
		ID:                "gemini-reasoning-high",
		ThinkingAliasBase: "gemini-reasoning",
	...
	_, errExecute := exec.Execute(
		context.Background(),
		auth,
		cliproxyexecutor.Request{
			Model:   "gemini-reasoning-high",
			Payload: []byte(`{"model":"gemini-reasoning-high","input":"hi"}`),
		},
	...
	if upstreamModel != "gemini-reasoning" {
		t.Fatalf("upstream model = %q, want gemini-reasoning", upstreamModel)
	}
}
