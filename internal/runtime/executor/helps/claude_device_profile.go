...
func ApplyClaudeLegacyDeviceHeaders(r *http.Request, ginHeaders http.Header, cfg *config.Config, confirmedClaudeCode bool) {
	...
	// Legacy confirmed clients honor explicit platform overrides. Unconfirmed
	// clients retain host-derived defaults unless configured away from the
	// canonical Claude Code platform.
	var headerDefaults config.ClaudeHeaderDefaults
	if cfg != nil {
		headerDefaults = cfg.ClaudeHeaderDefaults
	}
	legacyOS := mapStainlessOS()
	configuredOS := strings.TrimSpace(headerDefaults.OS)
	if configuredOS != "" && (confirmedClaudeCode || configuredOS != defaultClaudeFingerprintOS) {
		legacyOS = configuredOS
	}
	legacyArch := mapStainlessArch()
	configuredArch := strings.TrimSpace(headerDefaults.Arch)
	if configuredArch != "" && (confirmedClaudeCode || configuredArch != defaultClaudeFingerprintArch) {
		legacyArch = configuredArch
	}
	r.Header.Set("X-Stainless-Runtime-Version", profile.RuntimeVersion)
	r.Header.Set("X-Stainless-Package-Version", profile.PackageVersion)
