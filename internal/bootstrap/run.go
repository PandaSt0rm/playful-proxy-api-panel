...
type RuntimeBindings struct {
	...
	UsageStore    *usage.EventStore
	ProviderProbe *providerprobe.Service
	ConfigMirror  interface {
		PersistConfigBytes(context.Context, []byte, string) (string, error)
	}
}

type ProductFactory func(context.Context, RuntimeBindings) api.ManagementExtensionRegistrar
...
func Run(ctx context.Context, args []string, opts RunOptions) error {
	...
	buildinfo.Version = opts.BuildInfo.Version
	buildinfo.Commit = opts.BuildInfo.Commit
	buildinfo.BuildDate = opts.BuildInfo.BuildDate
	previousArgs := os.Args
	previousFlags := flag.CommandLine
	os.Args = append([]string{previousArgs[0]}, args...)
	flag.CommandLine = flag.NewFlagSet(previousArgs[0], flag.ContinueOnError)
	defer func() {
	...
	flag.StringVar(&configPath, "config", opts.DefaultConfigPath, "Configure File Path")
	...
	if bootstrapCfg := loadPluginBootstrapConfig(pluginBootstrapConfigPath(os.Args[1:], opts.DefaultConfigPath)); bootstrapCfg != nil {
	...
	if errParse := flag.CommandLine.Parse(args); errParse != nil {
		return errParse
	}
	...
	var runtimeService *cliproxy.Service
	if opts.ProductFactory != nil {
		serverOptions = append(serverOptions, api.WithManagementExtensionFactory(func(handler *management.Handler) api.ManagementExtensionRegistrar {
			return opts.ProductFactory(ctx, RuntimeBindings{
				Service:    runtimeService,
				Management: handler,
				UsageStore: usage.GetEventStore(),
			})
		}))
	}
	...
	if pluginHost.HasTriggeredCommandLineFlags() {
		if exitCode, handled := pluginHost.ExecuteCommandLine(context.Background(), os.Args[0], os.Args[1:], configFilePath, flag.CommandLine); handled {
			if exitCode != 0 {
				return fmt.Errorf("plugin command exited with status %d", exitCode)
			}
			return nil
		}
	}
	...
	if vertexImport != "" {
		...
		if tuiMode {
			if standalone {
				...
				captureService := func(service *cliproxy.Service) { runtimeService = service }
				cancel, done := cmd.StartServiceBackgroundWithPluginHost(cfg, configFilePath, password, pluginHost, captureService, serverOptions...)
			...
			startModelCatalogUpdaters(localModel, cfg.Home.Enabled)
			captureService := func(service *cliproxy.Service) { runtimeService = service }
			cmd.StartServiceWithPluginHost(cfg, configFilePath, password, pluginHost, captureService, serverOptions...)
		}
	}
	return nil
}
