...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	if errUnmarshal := json.Unmarshal(statsRR.Body.Bytes(), &statsPayload); errUnmarshal != nil || len(statsPayload.Usage) == 0 {
		t.Fatalf("usage stats payload missing usage object: err=%v body=%s", errUnmarshal, statsRR.Body.String())
	}
	if remaining := redisqueue.PopOldest(10); len(remaining) != 2 {
		t.Fatalf("usage stats request popped queue records = %d, want 2", len(remaining))
	} else {
