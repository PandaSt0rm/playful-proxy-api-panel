...
func TestRegisterModelsPrunesStaleClientAfterSnapshotChange(t *testing.T) {
	...
	storeTestSnapshot(host, capabilityRecord{
		id: "bravo",
		plugin: pluginapi.Plugin{Capabilities: pluginapi.Capabilities{
...
func TestAccessAdapterUnauthenticatedReturnsNotHandled(t *testing.T) {
	host := New()
	storeTestSnapshot(host, capabilityRecord{id: "auth-plugin"})
	adapter := &accessAdapter{
		host:     host,
		pluginID: "auth-plugin",
		path:     testPluginPath("auth-plugin"),
...
func TestAccessAdapterPanicFusesAndReturnsNotHandled(t *testing.T) {
	host := New()
	storeTestSnapshot(host, capabilityRecord{id: "auth-panic"})
	adapter := &accessAdapter{
		host:     host,
		pluginID: "auth-panic",
		path:     testPluginPath("auth-panic"),
		version:  testPluginVersion,
		provider: frontendAuthProviderFunc{
			identifier: "custom-auth",
			authenticate: func(ctx context.Context, req pluginapi.FrontendAuthRequest) (pluginapi.FrontendAuthResponse, error) {
...
func TestExecutorAdapterMethods(t *testing.T) {
	...
	adapter := &executorAdapter{
		...
		path:          testPluginPath("executor-plugin"),
		version:       testPluginVersion,
		provider:      "plugin-provider",
		executor:      exec,
...
func TestExecutorAdapterUsesResponseFormatForOutputTranslation(t *testing.T) {
	...
	host := New()
	storeTestSnapshot(host, capabilityRecord{id: "executor-plugin"})
	adapter := &executorAdapter{
		host:          host,
		pluginID:      "executor-plugin",
		path:          testPluginPath("executor-plugin"),
...
func TestExecutorAdapterPanicFusesAndReturnsError(t *testing.T) {
	host := New()
	storeTestSnapshot(host, capabilityRecord{id: "executor-panic"})
	calls := 0
	adapter := &executorAdapter{
		host:          host,
		pluginID:      "executor-panic",
		path:          testPluginPath("executor-panic"),
		version:       testPluginVersion,
		provider:      "plugin-provider",
		inputFormats:  []sdktranslator.Format{sdktranslator.FormatOpenAI},
		outputFormats: []sdktranslator.Format{sdktranslator.FormatOpenAI},
...
func newHostWithRecords(records ...capabilityRecord) *Host {
	host := New()
	storeTestSnapshot(host, records...)
	return host
}

func storeTestSnapshot(host *Host, records ...capabilityRecord) *Snapshot {
...
const testPluginVersion = "test-version"

func testPluginPath(id string) string {
	return "test-plugin-" + strings.TrimSpace(id) + ".so"
}
...
func normalizeTestCapabilityRecords(records []capabilityRecord) []capabilityRecord {
	for i := range records {
		...
		if strings.TrimSpace(records[i].path) == "" {
			records[i].path = testPluginPath(id)
		}
		if strings.TrimSpace(records[i].version) == "" {
			records[i].version = testPluginVersion
		}
	}
	sortRecords(records)
	return records
