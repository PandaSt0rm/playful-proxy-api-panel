...
type OpenAICompatibilityModel struct {
	...
	// static data first and then a legacy fallback catalog. Models without a
	// match get no automatic thinking-level variants.
	Thinking *registry.ThinkingSupport `yaml:"thinking,omitempty" json:"thinking,omitempty"`

	// ThinkingPayloads maps a canonical reasoning label to a JSON object merged
	// into the upstream request body when that label is requested.
