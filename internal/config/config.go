...
type Config struct {
	...
	// in memory for Management API consumers and the Redis RESP interface (LPOP/RPOP).
	// Default: 60. Max: 3600.
...
type OpenAICompatibilityModel struct {
	...
	// static data first and then a legacy fallback catalog (see
	// internal/thinking/catalog.go). Models without a match get no thinking
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	if err = yaml.Unmarshal(data, &cfg); err != nil {
		if optional {
			...
			cfgOptional := &Config{}
			cfgOptional.UpstreamConcurrency.Normalize()
