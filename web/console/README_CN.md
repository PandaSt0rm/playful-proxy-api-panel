...
用于管理与故障排查 **AIPROXY (AIPROXY)** / CLIProxyAPI 兼容后端的单文件 Web UI（React + TypeScript），通过 **Management API** 完成配置、凭据与日志等管理操作。

[English](README.md)

**主项目**: https://github.com/PandaSt0rm/aiproxy

**主维护位置**: 此前端维护在主仓的 `web/console` 目录中；旧的独立面板仓库会保留一段时间，方便用户过渡。
