...
| `proxy-url`, `passthrough-headers`, `disable-image-generation`, `enable-gemini-cli-endpoint`, `force-model-prefix`, `ws-auth`, `codex.identity-confuse`, `plugins.enabled`, `plugins.dir` | Config visual editor; dashboard shows image generation and WebSocket auth | `/config.yaml` plus existing simple endpoints where available | Network/request behavior is visible without source editing. `disable-image-generation` offers false/true/chat/passthrough. `codex.identity-confuse`, `plugins.enabled`, and `plugins.dir` are typed controls in the Network section. |
| `plugins.configs.<id>` | Plugins page: per-plugin enable toggle, metadata-driven config editor (priority, declared config fields), and delete action | `GET /v0/management/plugins`, `PATCH /v0/management/plugins/:id/enabled`, `PUT /v0/management/plugins/:id/config`, `DELETE /v0/management/plugins/:id` | Undeclared/advanced keys remain editable in the config source editor; the editor preserves keys it does not manage. |
| `plugins.store-sources` | Plugins page: plugin store browser (search, install, update) with registry source-error surfacing | `GET /v0/management/plugin-store`, `POST /v0/management/plugin-store/:id/install` | The built-in official registry is always included; extra registry URLs are added via `plugins.store-sources` in the config source editor. |
| `video-result-auth-cache-ttl` | Config source editor only | `/config.yaml` | Niche TTL binding returned video IDs to the creating credential; source-editable until it earns a typed control. |
...
| `sync-state-path` | Config source editor (source-only) | `/config.yaml` | Operational path for aiproxy-sync status reports (like `usage-statistics-path`); no typed control needed. The reported state itself is displayed on Tooling Templates via the sync-state endpoint. |
