...
export interface IndexedOpenRouterProvider {
  config: OpenAIProviderConfig;
  originalIndex: number;
}

interface OpenRouterSectionProps {
  providers: IndexedOpenRouterProvider[];
...
export function OpenRouterSection({
  ...
  const renderProviderCard = ({ config: provider, originalIndex }: IndexedOpenRouterProvider) => {
            ...
            <div className={styles.apiKeyEntriesSection}>
              ...
              <div className={styles.apiKeyEntryList}>
                {apiKeyEntries.map((entry, entryIndex) => {
                  ...
                  return (
                    <div
                      key={entry.authIndex || `openrouter-key-${originalIndex}-${entryIndex}`}
    ...
    <Card
      title={
        <span className={styles.cardTitle}>
          <img src={iconOpenRouter} alt="" className={styles.cardTitleIcon} />
          {t('ai_providers.openrouter_title')}
        </span>
      }
      extra={
        <Button size="sm" onClick={onAdd} disabled={actionsDisabled}>
          {t('ai_providers.openrouter_add_button')}
        </Button>
      }
    >
      {loading && sortedProviders.length === 0 ? (
        ...
        <EmptyState
          title={t('ai_providers.openrouter_empty_title')}
          description={t('ai_providers.openrouter_empty_desc')}
        />
      ) : (
