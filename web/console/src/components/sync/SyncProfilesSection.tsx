...
export function SyncProfilesSection() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showNotification(
        message ||
          t('sync_profiles.notifications.delete_error', {
      ...
      >
        <p className={styles.sectionHint}>
          {t('sync_profiles.section_hint', {
            defaultValue:
              'Sync profiles define which CLI tools to configure and which models/API keys to use. Use aiproxy-sync CLI to apply profiles.',
          })}
        </p>
        ...
        {!loading && error && (
          <div className={styles.errorContainer}>
            <span>
              {t('sync_profiles.load_error', { defaultValue: 'Failed to load sync profiles.' })}
            </span>
        ...
        {!loading && !error && profiles.length === 0 && (
          <EmptyState
            ...
            description={t('sync_profiles.empty_description', {
              defaultValue:
                'Create a sync profile to configure CLI tools with models from this proxy.',
            })}
            action={
