...
export type QuotaType = 'antigravity' | 'claude' | 'codex' | 'gemini-cli' | 'kimi' | 'zai' | 'xai';
...
const summarizeClaude = (state: ClaudeQuotaState, t: TFunction): QuotaSummary => {
  ...
  if (state.planType) {
    extras.push({
      ...
      value: t(`claude_quota.${state.planType}`),
    });
  }
  ...
  return {
    meters,
    extras,
    emptyMessageKey: meters.length ? undefined : 'claude_quota.empty_windows',
  };
};
...
const summarizeAntigravity = (state: AntigravityQuotaState): QuotaSummary => {
  ...
  return {
    meters,
    extras: [],
    emptyMessageKey: meters.length ? undefined : 'antigravity_quota.empty_models',
  };
};
...
const summarizeCodex = (state: CodexQuotaState, t: TFunction): QuotaSummary => {
  ...
  return {
    meters,
    extras,
    emptyMessageKey: meters.length ? undefined : 'codex_quota.empty_windows',
  };
};
...
const summarizeGeminiCli = (state: GeminiCliQuotaState, t: TFunction): QuotaSummary => {
  ...
  return {
    meters,
    extras,
    emptyMessageKey: meters.length ? undefined : 'gemini_cli_quota.empty_buckets',
  };
};
...
const fetchZaiQuota = async (file: AuthFileItem, t: TFunction): Promise<ZaiQuotaRow[]> => {
  const rawAuthIndex = file['auth_index'] ?? file.authIndex;
  const authIndex = normalizeAuthIndex(rawAuthIndex);
  if (!authIndex) {
...
const summarizeZai = (state: ZaiQuotaState, t: TFunction): QuotaSummary => {
  ...
  return {
    meters,
    extras: [],
    emptyMessageKey: meters.length ? undefined : 'zai_quota.empty_data',
  };
};
...
const summarizeKimi = (state: KimiQuotaState, t: TFunction): QuotaSummary => {
  ...
  return {
    meters,
    extras: [],
    emptyMessageKey: meters.length ? undefined : 'kimi_quota.empty_data',
  };
};
...
const fetchXaiQuota = async (file: AuthFileItem, t: TFunction): Promise<XaiQuotaFetchResult> => {
  ...
  if (
    ...
    const settingsPayload = parseXaiBillingPayload(settingsResult.body ?? settingsResult.bodyText);
    if (settingsPayload) {
