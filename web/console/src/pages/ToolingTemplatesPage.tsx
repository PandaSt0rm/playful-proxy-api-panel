                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        checked={checked}
                        onChange={() => (checked ? onRemove(model.name) : onAdd(model.name))}
                      />
                      <span className={styles.pickerRowName}>{model.name}</span>
                      {model.alias && <span className={styles.pickerRowAlias}>{model.alias}</span>}
                    </label>
                  );
                })
              )}
...
export function ToolingTemplatesPage() {
  ...
  const defaultBaseUrl = useMemo(() => authApiBase || detectApiBaseFromLocation(), [authApiBase]);
      ...
      .catch((error: unknown) => {
        ...
        setTemplateMetadataError(
          getErrorMessage(error) ||
            t('tooling_templates.templates_error', {
    ...
    <WorkspacePage title={t('nav.tooling_templates', { defaultValue: 'Tooling Templates' })}>
      ...
      <div className={styles.content}>
        ...
        {(() => {
          return (
            <Card
              title={t('tooling_templates.manual_config.title', { defaultValue: 'Manual config' })}
            >
        ...
        {(() => {
          ...
          return (
            <Card title={t('tooling_templates.curl_card.title', { defaultValue: 'curl examples' })}>
              <div className={styles.cardContent}>
                <p className={styles.cardHint}>
        ...
        {(() => {
          const activeCard = TAB_CARDS.find((card) => card.id === activeTab) ?? TAB_CARDS[0];
