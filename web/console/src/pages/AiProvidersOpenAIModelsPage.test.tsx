...
const buildContext = (overrides: Partial<OpenAIEditOutletContext> = {}): OpenAIEditOutletContext =>
  ({
    providerMode: 'openai',
    hasIndexParam: false,
    editIndex: null,
    ...
    <MemoryRouter initialEntries={['/ai-providers/openai/new/models']}>
      <Routes>
        <Route element={<Outlet context={context} />}>
          <Route path="/ai-providers/:mode/new/models" element={<AiProvidersOpenAIModelsPage />} />
        </Route>
      </Routes>
    </MemoryRouter>
  );
...
describe('AiProvidersOpenAIModelsPage', () => {
  ...
  it('shows the proxy toggle when the first key entry has a proxy URL', async () => {
    renderPage(
      buildContext({
        form: buildForm({
          apiKeyEntries: [buildKeyEntry({ apiKey: 'sk-1', proxyUrl: 'http://proxy:8080' })],
        }),
      })
    );
