...
type BreakdownTab =
  'models' | 'accounts' | 'endpoints' | 'failures' | 'realtime' | 'costs' | 'data';
...
const createHourKey = (date: Date): string =>
  `${createDateKey(date)} ${String(date.getHours()).padStart(2, '0')}:00`;
...
const findCustomPricing = (
  modelName: string,
  prices: UsageModelPrice[]
): { model: string; rate: PricingRate } | null => {
...
const calculateCost = (
  modelName: string,
  tokens: UsageTokenStats | undefined,
  prices: UsageModelPrice[]
) => {
  const pricing = findCustomPricing(modelName, prices);
...
const normalizeTokens = (tokens?: UsageTokenStats): UsageTokenStats => {
  ...
  const total =
    safeNumber(tokens?.total_tokens) ||
...
const newAggregateRow = (
  id: string,
  label: string,
  extra?: Partial<AggregateRow>
): AggregateRow => ({
  id,
  label,
  requests: 0,
...
const eventKeyName = (event: UsageEvent): string =>
  event.api_key_alias ||
  event.auth_index ||
  (event.api_key_hash ? event.api_key_hash.slice(0, 12) : '') ||
  event.source ||
  event.endpoint ||
  '-';
...
const summarizeActivityWindow = (
  ...
  const scoped = records.filter(
    (record) => record.timestampMs > 0 && record.timestampMs >= startMs
  );
  const requests = scoped.length;
  const failures = scoped.filter((record) => record.detail.failed).length;
  const tokens = scoped.reduce(
    (total, record) => total + safeNumber(record.detail.tokens?.total_tokens),
    0
  );
...
const makeTrendBuckets = (records: DetailRecord[], span: TrendSpan): TrendPoint[] => {
  ...
  if (span === '24h') {
    for (let i = 23; i >= 0; i -= 1)
      addBucket(createHourKey(new Date(now.getTime() - i * 60 * 60 * 1000)));
  } else if (span === '7d' || span === '14d' || span === '30d') {
    const days = span === '7d' ? 6 : span === '14d' ? 13 : 29;
    for (let i = days; i >= 0; i -= 1)
      addBucket(createDateKey(new Date(now.getFullYear(), now.getMonth(), now.getDate() - i)));
  }
...
const authIndexFromFile = (file: AuthFileItem): string | null =>
...
function TrendChart({
  ...
  const values = rows.map((row) =>
    metric === 'requests' ? row.requests : metric === 'tokens' ? row.tokens : row.cost
  );
  ...
  const points = values.map((value, index) => {
    const x =
      paddingX + (rows.length <= 1 ? plotWidth / 2 : (index / (rows.length - 1)) * plotWidth);
...
export function UsagePage() {
  ...
  const allRecords = useMemo(
    () =>
      events.length
        ? events.map((event) => eventToDetailRecord(event, modelPrices))
        : flattenSnapshotDetails(usage, modelPrices),
  ...
  const totalInputTokens = filteredRecords.length
    ? filteredRecords.reduce(
        (total, record) => total + safeNumber(record.detail.tokens?.input_tokens),
        0
      )
    : safeNumber(usage.total_input_tokens);
  const totalCachedTokens = filteredRecords.length
    ? filteredRecords.reduce(
        (total, record) => total + safeNumber(record.detail.tokens?.cached_tokens),
        0
      )
    : safeNumber(usage.total_cached_tokens);
  const totalOutputTokens = filteredRecords.length
    ? filteredRecords.reduce(
        (total, record) => total + safeNumber(record.detail.tokens?.output_tokens),
        0
      )
  ...
  const unpricedModels = useMemo(
    () =>
      Array.from(
        new Set(
          filteredRecords.filter((record) => record.cost === null).map((record) => record.modelName)
        )
      ).sort(),
    [filteredRecords]
  );
  ...
  }, [systemModels, unpricedModels, filteredRecords, modelPrices]);

  const filteredPrices = useMemo(
    () =>
      priceFilter.trim()
        ? modelPrices.filter((p) =>
            p.model.toLowerCase().includes(priceFilter.trim().toLowerCase())
          )
        : modelPrices,
  ...
  useEffect(() => {
    if (
      ...
      const preferredKey = configKeys.length
        ? String(configKeys[0]).trim()
        : managementKey?.trim() || undefined;
  ...
  const hasUsage =
    totalRequests > 0 || Object.keys(usage.apis ?? {}).length > 0 || events.length > 0;
  ...
  const codexLocalUsageByAuthIndex = useMemo(() => {
    ...
    filteredRecords.forEach((record) => {
      ...
      const row =
        rows.get(authIndex) ??
        newAggregateRow(authIndex, record.keyName, {
          keyName: record.keyName,
          apiName: record.provider,
        });
      addDetailToAggregate(row, record);
  ...
  const recentRows = useMemo(
    () => [...filteredRecords].sort((a, b) => b.timestampMs - a.timestampMs).slice(0, 120),
    [filteredRecords]
  );
  ...
  const activityWindowSummaries = useMemo(
    () =>
      activityWindowDefinitions.map((definition) =>
        summarizeActivityWindow(filteredRecords, definition, nowMs)
      ),
    [filteredRecords, nowMs]
  );
  const recentHourBuckets = useMemo(
    () => makeRecentHourBuckets(filteredRecords, 24, nowMs),
    [filteredRecords, nowMs]
  );
  const tokenBreakdown = useMemo(
    () => summarizeTokenBreakdown(filteredRecords, usage),
    [filteredRecords, usage]
  );
  const trendRows = useMemo(
    () => makeTrendBuckets(filteredRecords, timeRange),
  ...
  const cacheHitRate =
    totalInputTokens > 0
      ? (totalCachedTokens / totalInputTokens) * 100
      : safeNumber(usage.cache_hit_rate);
  const lastSeenMs = filteredRecords.reduce(
    (latest, record) => Math.max(latest, record.timestampMs),
    0
  );
  const activeDays =
    new Set(
      filteredRecords
        .map((record) =>
          record.timestampMs > 0 ? createDateKey(new Date(record.timestampMs)) : ''
        )
        .filter(Boolean)
    ).size || Object.keys(usage.requests_by_day ?? {}).length;
  const activeHours =
    new Set(
      filteredRecords
        .map((record) =>
          record.timestampMs > 0 ? createHourKey(new Date(record.timestampMs)) : ''
        )
        .filter(Boolean)
    ).size || Object.keys(usage.requests_by_hour ?? {}).length;
  ...
  const averageFirstByteMs = filteredRecords.length
    ? average(
        filteredRecords.reduce(
          (total, record) => total + safeNumber(record.detail.first_byte_latency_ms),
          0
        ),
        filteredRecords.filter((record) => safeNumber(record.detail.first_byte_latency_ms) > 0)
          .length
      )
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `aiproxy-usage-${fileSafeDate}.json`;
  ...
  const handleExportEvents = async () => {
    setExporting(true);
    try {
      const responseData = await usageApi.exportEvents({
        ...timeRangeParams(timeRange),
        limit: 200000,
      });
      const blob =
        responseData.data instanceof Blob
          ? responseData.data
          : new Blob([responseData.data], { type: 'application/x-ndjson;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = `aiproxy-usage-events-${new Date().toISOString().replace(/[:.]/g, '-')}.jsonl`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
  ...
  const handleImportFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showNotification(
        t('usage_statistics.import_success', { added: result.added, skipped: result.skipped }),
        'success'
      );
      await loadUsage();
    } catch (err: unknown) {
  ...
  const handleSyncPrices = async () => {
    ...
    try {
      ...
      showNotification(
        t('usage_statistics.price_sync_success', { count: result.saved }),
        'success'
      );
      await loadUsage();
    } catch (err: unknown) {
  ...
  const handleSaveAlias = async () => {
    ...
    setSavingAlias(true);
    try {
      await usageApi.saveAPIKeyAlias({
        api_key_hash: normalizedHash,
  ...
  const handleDeleteAlias = async (hash: string) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : t('usage_statistics.alias_delete_failed');
      showNotification(message, 'error');
    }
  };
  ...
  const loadCodexQuotaContext = useCallback(async () => {
    ...
    try {
      ...
      showNotification(
        t('usage_statistics.codex_quota_loaded', { count: files.length }),
        'success'
      );
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : t('usage_statistics.codex_quota_load_failed');
      showNotification(message, 'error');
    } finally {
  ...
  const renderAggregateTable = (
    rows: AggregateRow[],
    mode: 'models' | 'accounts' | 'endpoints' | 'failures'
  ) => (
      ...
      <table className={styles.table}>
        <thead>
          <tr>
            ...
            <th>
              {mode === 'models' ? t('usage_statistics.accounts') : t('usage_statistics.models')}
            </th>
          </tr>
        </thead>
        <tbody>
            ...
            rows.map((row) => (
              <tr key={row.id}>
                <td className={styles.primaryCell}>
                  ...
                  {row.apiName && mode !== 'endpoints' ? (
                    <span className={styles.cellHint}>{row.apiName}</span>
                  ) : null}
                </td>
              ...
              <tr key={row.id}>
                ...
                <td>
                  <span className={row.detail.failed ? styles.statusFailed : styles.statusOk}>
                    {row.detail.failed
                      ? `${row.statusCode || '-'} ${t('usage_statistics.failed')}`
                      : t('usage_statistics.success')}
                  </span>
                </td>
  ...
  const renderQuotaStatus = (quota?: CodexQuotaState) => {
    if (quota?.status === 'loading')
      return <span className={styles.statusPill}>{t('usage_statistics.quota_loading')}</span>;
    if (quota?.status === 'success')
      return <span className={styles.statusOk}>{t('usage_statistics.success')}</span>;
  ...
  const renderQuotaWindows = (quota?: CodexQuotaState) => {
    if (!quota || quota.status === 'idle')
      return <div className={styles.quotaMessage}>{t('usage_statistics.quota_idle')}</div>;
    if (quota.status === 'loading')
      return <div className={styles.quotaMessage}>{t('usage_statistics.quota_loading')}</div>;
    if (quota.status === 'error')
      return (
        <div className={styles.quotaMessage}>
          {quota.error || t('usage_statistics.quota_error')}
        </div>
      );
    if (!quota.windows.length)
      return <div className={styles.quotaMessage}>{t('codex_quota.empty_windows')}</div>;
    ...
    <div className={styles.managementPanel}>
      <div className={styles.panelHeaderRow}>
        ...
        <Button
          ...
          variant="secondary"
          size="sm"
          onClick={loadCodexQuotaContext}
          loading={loadingCodexQuota}
      ...
      {codexQuotaLoaded && codexQuotaRows.length ? (
          ...
          {codexQuotaRows.map(({ file, authIndex, quota, localUsage }) => (
            <div key={file.name} className={styles.quotaContextRow}>
              <div className={styles.quotaIdentity}>
                ...
                <span>
                  {authIndex
                    ? `${t('usage_statistics.account')}: ${authIndex}`
                    : t('codex_quota.missing_auth_index')}
                </span>
              </div>
        ...
        <div className={styles.emptyInline}>
          {codexQuotaLoaded
            ? t('usage_statistics.no_codex_accounts')
            : t('usage_statistics.quota_idle')}
        </div>
      )}
    </div>
  );
  ...
  return (
      ...
      actions={
        <div className={styles.headerActions}>
          ...
          <Button
            ...
            loading={exporting}
            disabled={connectionStatus !== 'connected' || !status.enabled}
          >
    ...
    >
      ...
      <Card>
        <div className={styles.controlPanel}>
            ...
            <div className={styles.segmentedControl}>
              {(['24h', '7d', '14d', '30d', 'all'] as TrendSpan[]).map((span) => (
                <button
                  type="button"
                  key={span}
                  className={timeRange === span ? styles.segmentActive : ''}
                  onClick={() => setTimeRange(span)}
                >
          ...
          <label className={styles.controlField}>
            ...
            <select
              value={autoRefresh}
              onChange={(event) => setAutoRefresh(event.currentTarget.value as AutoRefresh)}
            >
          ...
          <label className={styles.searchField}>
            ...
            <input
              ...
              placeholder={t('usage_statistics.search_placeholder')}
            />
          </label>
        </div>
      </Card>
      ...
      <section className={styles.metricGrid} aria-label={t('usage_statistics.summary')}>
        {renderMetric(
          t('usage_statistics.total_requests'),
          formatNumber(totalRequests),
          `${formatNumber(totalRequests - failedRequests)} ${t('usage_statistics.success')} / ${formatNumber(failedRequests)} ${t('usage_statistics.failed')}`,
          <IconSatellite size={16} />
        )}
        {renderMetric(
          t('usage_statistics.estimated_cost'),
          formatCost(estimatedTotalCost),
          t('usage_statistics.priced_requests', { count: pricedRequestCount }),
          <IconDollarSign size={16} />
        )}
        {renderMetric(
          t('usage_statistics.cache_hit_rate'),
          formatPercent(cacheHitRate),
          `${formatNumber(totalCachedTokens)} ${t('usage_statistics.cached_tokens')}`,
          <IconShield size={16} />
        )}
        {renderMetric(
          t('usage_statistics.first_byte_latency'),
          formatLatency(averageFirstByteMs),
          t('usage_statistics.average_value'),
          <IconTimer size={16} />
        )}
        {renderMetric(
          t('usage_statistics.average_latency'),
          formatLatency(averageLatencyMs),
          t('usage_statistics.average_value'),
          <IconTimer size={16} />
        )}
        {renderMetric(
          t('usage_statistics.total_tokens'),
          formatNumber(totalTokens),
          `${formatNumber(totalInputTokens)} in / ${formatNumber(totalOutputTokens)} out`,
          <IconDiamond size={16} />
        )}
        {renderMetric(
          t('usage_statistics.recent_24h'),
          formatNumber(last24hSummary?.requests),
          t('usage_statistics.recent_window_sub', {
            tokens: formatCompact(last24hSummary?.tokens),
            failed: formatNumber(last24hSummary?.failures),
          }),
          <IconChartLine size={16} />
        )}
        {renderMetric(
          t('usage_statistics.failure_rate'),
          formatPercent(failedRate),
          `${formatNumber(failedRequests)} / ${formatNumber(totalRequests)}`,
          <IconShield size={16} />
        )}
        {renderMetric(
          t('usage_statistics.active_dimensions'),
          `${formatNumber(apiRows.length)} / ${formatNumber(modelRows.length)} / ${formatNumber(accountRows.length)}`,
          t('usage_statistics.active_dimensions_sub'),
          <IconModelCluster size={16} />
        )}
        {renderMetric(
          t('usage_statistics.event_store'),
          status.enabled ? formatNumber(status.event_count) : t('usage_statistics.memory_only'),
          status.path || t('usage_statistics.no_sqlite_store'),
          <IconFilterAll size={16} />
        )}
        {renderMetric(
          t('usage_statistics.last_request'),
          formatTimestamp(lastSeenMs),
          t('usage_statistics.active_period_sub', {
            days: formatNumber(activeDays),
            hours: formatNumber(activeHours),
          }),
          <IconTimer size={16} />
        )}
        {renderMetric(
          t('usage_statistics.top_model'),
          topModel,
          t('usage_statistics.by_requests'),
          <IconTrendingUp size={16} />
        )}
        {renderMetric(
          t('usage_statistics.top_key'),
          topKey,
          t('usage_statistics.by_requests'),
          <IconKey size={16} />
        )}
        {renderMetric(
          t('usage_statistics.top_endpoint'),
          topEndpoint,
          t('usage_statistics.by_requests'),
          <IconChartLine size={16} />
        )}
      ...
      <div className={styles.insightGrid}>
        <Card title={t('usage_statistics.recent_activity_title')}>
          <div className={styles.activityWindowGrid}>
            {activityWindowSummaries.map((summary) => (
              <div key={summary.key} className={styles.activityWindow}>
                <span className={styles.activityWindowLabel}>
                  {t(`usage_statistics.activity_window_${summary.key}`)}
                </span>
                <strong>{formatNumber(summary.requests)}</strong>
                <span>
                  {t('usage_statistics.recent_window_sub', {
                    tokens: formatCompact(summary.tokens),
                    failed: formatNumber(summary.failures),
                  })}
                </span>
                <span>
                  {t('usage_statistics.rate_window_sub', {
                    rpm: formatRate(summary.rpm),
                    tpm: formatRate(summary.tpm),
                  })}
                </span>
              </div>
            ))}
          ...
          <div
            ...
            aria-label={t('usage_statistics.recent_activity_title')}
          >
            {recentHourBuckets.map((bucket) => {
              const height =
                bucket.requests > 0 ? 16 + (bucket.requests / recentActivityMax) * 84 : 6;
              return (
        ...
        <Card title={t('usage_statistics.health_title')}>
          <div className={styles.healthSummary}>
            <div className={styles.healthRateBlock}>
              ...
              <small>
                {formatNumber(totalRequests - failedRequests)} {t('usage_statistics.success')} /{' '}
                {formatNumber(failedRequests)} {t('usage_statistics.failed')}
              </small>
            </div>
            <div className={styles.healthTrack}>
              <span
                ...
                style={{
                  width: `${totalRequests > 0 ? ((totalRequests - failedRequests) / totalRequests) * 100 : 0}%`,
                }}
              />
              <span
        ...
        <Card title={t('usage_statistics.token_breakdown_title')}>
          <div className={styles.breakdownList}>
            {tokenBreakdownItems.map((item) => {
              const percentage =
                tokenBreakdownTotal > 0 ? (item.value / tokenBreakdownTotal) * 100 : 0;
              return (
                <div key={item.key} className={styles.breakdownRow}>
                  ...
                  <div className={styles.breakdownTrack}>
                    <span
                      className={`${styles.breakdownFill} ${item.className}`}
        ...
        <Card title={t('usage_statistics.top_dimensions_title')}>
          <div className={styles.topLists}>
            {[
              {
                key: 'models',
                title: t('usage_statistics.top_models'),
                rows: modelRows.slice(0, 5),
              },
              {
                key: 'endpoints',
                title: t('usage_statistics.top_endpoints'),
                rows: endpointRows.slice(0, 5),
              },
              {
                key: 'accounts',
                title: t('usage_statistics.top_accounts'),
                rows: accountRows.slice(0, 5),
              },
            ].map((group) => {
                    ...
                    group.rows.map((row) => (
                      <div key={`${group.key}-${row.id}`} className={styles.topListRow}>
                        <div className={styles.topListMeta}>
                          <span className={styles.monoCell}>{row.label}</span>
                          <small>
                            {formatNumber(row.requests)} {t('usage_statistics.requests')} -{' '}
                            {formatCompact(row.tokens)} {t('usage_statistics.tokens')}
                          </small>
                        </div>
            ...
            <div className={styles.managementPanel}>
              ...
              <div className={styles.formGrid}>
                ...
                <input
                  ...
                  placeholder={t('usage_statistics.price_input')}
                  inputMode="decimal"
                />
                <input
                  value={priceCached}
                  onChange={(event) => setPriceCached(event.currentTarget.value)}
                  placeholder={t('usage_statistics.price_cached')}
                  inputMode="decimal"
                />
                <input
                  value={priceOutput}
                  onChange={(event) => setPriceOutput(event.currentTarget.value)}
                  placeholder={t('usage_statistics.price_output')}
                  inputMode="decimal"
                />
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={clearPriceForm}
                    disabled={!status.enabled}
                    title={t('usage_statistics.clear_form')}
                  >
                    <IconX size={14} />
                  </Button>
                </div>
              ...
              <div className={styles.pillList}>
                {unpricedModels.map((model) => (
                  <button type="button" key={model} onClick={() => setPriceModel(model)}>
                    {model}
                  </button>
                ))}
                  ...
                  {!filteredPrices.length && (
                    <tr>
                      <td colSpan={5} className={styles.emptyCell}>
                        {t('usage_statistics.no_model_prices')}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className={styles.managementGrid}>
            <div className={styles.managementPanel}>
                ...
                <div>
                  ...
                  <strong>
                    {status.retention_days
                      ? formatNumber(status.retention_days)
                      : t('usage_statistics.retention_unlimited')}
                  </strong>
                </div>
                <div>
              ...
              <div className={styles.dataActions}>
                ...
                <Button
                  ...
                  loading={pruning}
                  disabled={!status.enabled || !status.retention_days}
                >
                  <IconTrash2 size={15} />
              ...
              <p className={styles.panelHint}>
                {status.retention_days
                  ? t('usage_statistics.retention_cleanup_hint', {
                      days: formatNumber(status.retention_days),
                    })
            ...
            <div className={styles.managementPanel}>
              ...
              <div className={styles.formGrid}>
                <input
                  value={aliasHash}
                  onChange={(event) => setAliasHash(event.currentTarget.value)}
                  placeholder={t('usage_statistics.api_key_hash')}
                />
                <input
                  value={aliasName}
                  onChange={(event) => setAliasName(event.currentTarget.value)}
                  placeholder={t('usage_statistics.alias')}
                />
                <Button
                  type="button"
                  variant="primary"
              ...
              <div className={styles.aliasList}>
                {aliases.map((alias) => (
                  <div key={alias.api_key_hash} className={styles.aliasRow}>
                    ...
                    <button
                      type="button"
                      onClick={() => handleDeleteAlias(alias.api_key_hash)}
                      aria-label={t('usage_statistics.delete_alias')}
                    >
                      <IconTrash2 size={15} />
                    </button>
                  </div>
                ...
                {!aliases.length && (
                  <div className={styles.emptyInline}>{t('usage_statistics.no_aliases')}</div>
                )}
              </div>
            </div>
          </div>
