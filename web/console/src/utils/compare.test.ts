...
describe('areKeyValueEntriesEqual', () => {
  ...
  it('returns true for distinct arrays with identical entries in the same order', () => {
    expect(areKeyValueEntriesEqual([{ key: 'a', value: '1' }], [{ key: 'a', value: '1' }])).toBe(
      true
    );
  });
  ...
  it('returns false when a key differs', () => {
    expect(areKeyValueEntriesEqual([{ key: 'a', value: '1' }], [{ key: 'b', value: '1' }])).toBe(
      false
    );
  });

  it('returns false when a value differs', () => {
    expect(areKeyValueEntriesEqual([{ key: 'a', value: '1' }], [{ key: 'a', value: '2' }])).toBe(
      false
    );
  });
...
describe('areModelEntriesEqual', () => {
  ...
  it('returns true for distinct entries with matching name and alias', () => {
    expect(
      areModelEntriesEqual([{ name: 'gpt-4', alias: 'GPT 4' }], [{ name: 'gpt-4', alias: 'GPT 4' }])
    ).toBe(true);
  });
  ...
  it('returns false when a name differs', () => {
    expect(
      areModelEntriesEqual([{ name: 'gpt-4', alias: 'a' }], [{ name: 'gpt-3', alias: 'a' }])
    ).toBe(false);
  });

  it('returns false when an alias differs', () => {
    expect(
      areModelEntriesEqual([{ name: 'gpt-4', alias: 'a' }], [{ name: 'gpt-4', alias: 'b' }])
    ).toBe(false);
  });

  it('treats missing regex and explicit false as equal', () => {
    expect(
      areModelEntriesEqual([{ name: 'm', alias: 'a' }], [{ name: 'm', alias: 'a', regex: false }])
    ).toBe(true);
  });
  ...
  it('treats missing thinking and explicit null as equal', () => {
    expect(
      areModelEntriesEqual([{ name: 'm', alias: 'a' }], [{ name: 'm', alias: 'a', thinking: null }])
    ).toBe(true);
  });
