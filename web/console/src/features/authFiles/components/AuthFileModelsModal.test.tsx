...
import { AuthFileModelsModal, type AuthFileModelsModalProps } from './AuthFileModelsModal';
...
describe('AuthFileModelsModal content states', () => {
  ...
  it('renders each model id in the list', () => {
    render(<AuthFileModelsModal {...baseProps({ models: [{ id: 'gpt-4o' }, { id: 'gpt-4' }] })} />);
  ...
  it('renders the display name when it differs from the model id', () => {
    render(
      <AuthFileModelsModal {...baseProps({ models: [{ id: 'gpt-4o', display_name: 'GPT-4o' }] })} />
    );
