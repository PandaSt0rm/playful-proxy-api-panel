...
describe('useAuthStore.login — success path', () => {
  ...
  it('normalizes the apiBase before storing it', async () => {
    await useAuthStore
      .getState()
      .login({ apiBase: 'localhost:8317/v0/management', managementKey: 'secret' });
  ...
  it('persists isLoggedIn when rememberPassword is true', async () => {
    await useAuthStore
      .getState()
      .login({ apiBase: 'localhost:8317', managementKey: 'secret', rememberPassword: true });
  ...
  it('does not persist isLoggedIn when rememberPassword is false', async () => {
    await useAuthStore
      .getState()
      .login({ apiBase: 'localhost:8317', managementKey: 'secret', rememberPassword: false });
  ...
  it('clears the models cache on login', async () => {
    useModelsStore.setState({
      cache: { data: [], timestamp: Date.now(), apiBase: 'x', apiKey: 'y' },
      models: [{ id: 'm' }] as never,
    });
...
describe('useAuthStore.login — failure path', () => {
  ...
  it('does not persist isLoggedIn after a failed login', async () => {
    ...
    await expect(
      useAuthStore
        .getState()
        .login({ apiBase: 'localhost:8317', managementKey: 'bad', rememberPassword: true })
    ).rejects.toThrow();
...
describe('useAuthStore.logout', () => {
  ...
  it('clears the server version metadata', () => {
    ...
    expect([
      useAuthStore.getState().serverVersion,
      useAuthStore.getState().serverBuildDate,
    ]).toEqual([null, null]);
  });
  ...
  it('updates the server version when a server-version-update event is dispatched', () => {
    window.dispatchEvent(
      new CustomEvent('server-version-update', {
        detail: { version: '9.9.9', buildDate: '2026-12-31' },
      })
    );
