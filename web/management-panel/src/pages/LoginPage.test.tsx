...
const loginSpy = vi.fn<(c: { apiBase: string; managementKey: string; rememberPassword: boolean }) => Promise<void>>();
...
describe('LoginPage error mapping', () => {
  ...
  it('maps an ECONNABORTED code to the timeout message', async () => {
    ...
    expect(await screen.findByText('Connection timed out, server not responding')).toBeInTheDocument();
  });
  ...
  it('maps an ERR_NETWORK code to the network message', async () => {
    ...
    expect(
      await screen.findByText('Network connection failed, please check your network or server address')
    ).toBeInTheDocument();
  });
  ...
  it('falls back to the generic invalid message for an unclassified error', async () => {
    ...
    expect(await screen.findByText('Connection failed, please check address and key')).toBeInTheDocument();
  });
...
describe('LoginPage auto-login success', () => {
  it('keeps the splash visible while the post-login animation runs', async () => {
    vi.useFakeTimers();
    restoreSessionSpy.mockResolvedValue(true);

    renderLogin();
    // Flush the restoreSession microtask without advancing the 1500ms navigate.
  ...
  it('does not navigate before the animation delay elapses', async () => {
    vi.useFakeTimers();
    restoreSessionSpy.mockResolvedValue(true);
  ...
  it('navigates to the home route after the animation delay elapses', async () => {
    vi.useFakeTimers();
    restoreSessionSpy.mockResolvedValue(true);
