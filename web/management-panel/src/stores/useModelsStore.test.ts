...
describe('useModelsStore', () => {
  ...
  describe('isCacheValid', () => {
    ...
    it('returns false when the cached apiBase differs', () => {
      useModelsStore.setState({
        cache: { data: sampleModels, timestamp: Date.now(), apiBase: 'https://a.com', apiKey: 'key' },
      });
    ...
    it('returns false when the cached apiKey differs', () => {
      useModelsStore.setState({
        cache: { data: sampleModels, timestamp: Date.now(), apiBase: 'https://a.com', apiKey: 'key-1' },
      });
  ...
  describe('fetchModels (error path)', () => {
    it('rejects with the original error', async () => {
      ...
      await expect(useModelsStore.getState().fetchModels('https://a.com', 'key')).rejects.toBe(failure);
    });
