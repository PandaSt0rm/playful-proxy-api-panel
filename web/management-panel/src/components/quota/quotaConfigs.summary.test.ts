...
describe('getSummary normalizers', () => {
  ...
  it('gemini surfaces tier/credits and bucket amounts', () => {
    const state: GeminiCliQuotaState = {
      status: 'success',
      buckets: [
        { id: 'b', label: 'Flash', remainingFraction: 0.5, remainingAmount: 120, resetTime: undefined, tokenType: null },
      ],
      tierLabel: 'Ultra',
    ...
    expect(summary.extras).toEqual([
      ...
      { id: 'credits', label: 'gemini_cli_quota.credit_label', value: 'gemini_cli_quota.credit_amount:300' },
    ]);
  });
