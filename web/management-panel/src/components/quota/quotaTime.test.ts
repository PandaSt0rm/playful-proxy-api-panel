...
describe('formatRelativeTime', () => {
  ...
  it('reports hours under a day', () => {
    expect(formatRelativeTime(t, NOW - 3 * 3_600_000, NOW)).toBe('quota_management.updated_hours:3');
  });

  it('reports days beyond 24 hours', () => {
    expect(formatRelativeTime(t, NOW - 2 * 86_400_000, NOW)).toBe('quota_management.updated_days:2');
  });
});
