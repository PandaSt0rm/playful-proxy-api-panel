...
export function formatRelativeTime(t: TFunction, fromMs: number, nowMs: number): string {
  ...
  if (diff < MINUTE) return t('quota_management.updated_seconds', { count: Math.floor(diff / SECOND) });
  if (diff < HOUR) return t('quota_management.updated_minutes', { count: Math.floor(diff / MINUTE) });
