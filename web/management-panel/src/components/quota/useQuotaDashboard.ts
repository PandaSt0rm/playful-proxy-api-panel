...
function writeCredentialState(credential: QuotaCredential, next: QuotaStatusState): void {
  const setter = useQuotaStore.getState()[credential.config.storeSetter] as unknown as AnyQuotaSetter;
...
export function useQuotaDashboard(files: AuthFileItem[], filesLoading: boolean, disabled: boolean) {
  ...
  const credentialViews = useMemo<QuotaCredentialView[]>(() => {
    return credentials.map((credential) => {
      ...
      const summary =
        state && status === 'success' ? credential.config.getSummary(state, t) : EMPTY_QUOTA_SUMMARY;
      return {
        ...credential,
        status,
  ...
  const refreshOne = useCallback(
    async (key: string): Promise<void> => {
      ...
      const status = await fetchOne(credential);
      if (status === 'success') {
        showNotification(t('auth_files.quota_refresh_success', { name: credential.name }), 'success');
