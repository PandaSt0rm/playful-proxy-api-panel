...
describe('HeaderInputList', () => {
  ...
  it('emits the updated value for the edited row, preserving its key', async () => {
    ...
    const user = userEvent.setup();
    render(
      <HeaderInputList entries={[{ key: 'X-Api-Key', value: 'old' }]} onChange={onChange} addLabel="Add" />
    );
  ...
  it('does not emit when clicking the disabled single-row remove button', async () => {
    ...
    const user = userEvent.setup();
    render(
      <HeaderInputList entries={[{ key: 'X-Solo', value: 'v' }]} onChange={onChange} addLabel="Add" />
    );
  ...
  it('disables the remove button when only a single row remains', () => {
    render(
      <HeaderInputList entries={[{ key: 'X-Solo', value: 'v' }]} onChange={() => {}} addLabel="Add" />
    );
