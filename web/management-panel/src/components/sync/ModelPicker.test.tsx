...
describe('ModelPicker — active (single-select) mode', () => {
  it('shows the placeholder when no value is selected', () => {
    render(
      <ModelPicker mode="active" value="" onChange={vi.fn()} groups={GROUPS} placeholder="Pick one" />
    );
  ...
  it('opens the popover dialog when the trigger is clicked', async () => {
    render(<ModelPicker mode="active" value="" onChange={vi.fn()} groups={GROUPS} ariaLabel="Active model" />);
...
describe('ModelPicker — search filtering', () => {
  ...
  it('shows the empty hint when no model matches the query', async () => {
    render(
      <ModelPicker mode="active" value="" onChange={vi.fn()} groups={GROUPS} emptyHint="Nothing here" />
    );
    await openPicker();
...
describe('ModelPicker — filter-list (multi-select) mode', () => {
  it('shows the add-models placeholder when nothing is selected', () => {
    render(
      <ModelPicker mode="filter-list" values={[]} onChange={vi.fn()} groups={GROUPS} placeholder="Add models" />
    );
  ...
  it('shows a singular count label when exactly one model is selected', () => {
    render(<ModelPicker mode="filter-list" values={['gpt-5']} onChange={vi.fn()} groups={GROUPS} />);
  ...
  it('shows a plural count label when multiple models are selected', () => {
    render(
      <ModelPicker mode="filter-list" values={['gpt-5', 'gpt-5-mini']} onChange={vi.fn()} groups={GROUPS} />
    );
  ...
  it('renders each selected value as a chip', () => {
    render(
      <ModelPicker mode="filter-list" values={['gpt-5', 'claude-opus-4']} onChange={vi.fn()} groups={GROUPS} />
    );
  ...
  it('adds a model to the selection when an unchecked row is clicked', async () => {
    const onChange = vi.fn();
    render(<ModelPicker mode="filter-list" values={['gpt-5']} onChange={onChange} groups={GROUPS} />);
    await openPicker();
  ...
  it('removes a model from the selection when a checked row is clicked', async () => {
    const onChange = vi.fn();
    render(
      <ModelPicker mode="filter-list" values={['gpt-5', 'gpt-5-mini']} onChange={onChange} groups={GROUPS} />
    );
    await openPicker();

    await userEvent.click(within(screen.getByRole('dialog')).getByRole('checkbox', { name: 'gpt-5' }));
  ...
  it('marks a selected row as aria-checked', async () => {
    ...
    expect(within(screen.getByRole('dialog')).getByRole('checkbox', { name: 'gpt-5', checked: true })).toBeInTheDocument();
  });
