      ...
      >
            ...
            filteredGroups.map((g) => (
              <div key={g.key} className={styles.modelGroup}>
                <div className={styles.modelGroupTitle}>
                  <span>{g.label}</span>
                  {g.sublabel && (
                    <span className={styles.modelGroupSublabel}>{g.sublabel}</span>
                  )}
                </div>
                <ul className={styles.modelGroupList}>
                  {g.models.map((m) => {
                    const active =
                      props.mode === 'active' ? props.value === m : selectedSet.has(m);
                    return (
                      <li key={m}>
                        <button
                          type="button"
                          ...
                          aria-checked={
                            props.mode === 'filter-list' ? active : undefined
                          }
                          aria-selected={
                            props.mode === 'active' ? active : undefined
                          }
                        >
        ...
        {props.mode === 'filter-list' && props.values.length > 0 && (
          <div className={styles.modelPickerFooter}>
            <button
              type="button"
              className={styles.modelPickerClearLink}
              onClick={clearAll}
            >
              Clear all
            </button>
            <span className={styles.modelPickerCount}>
