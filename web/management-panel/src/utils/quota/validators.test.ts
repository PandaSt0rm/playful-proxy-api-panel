...
describe('isZaiFile', () => {
  ...
  it('returns true when the base url points at the zai host', () => {
    const result = isZaiFile(makeFile({ provider: 'openai', baseUrl: 'https://api.z.ai/api/coding' }));
  ...
  it('returns false for an unrelated provider and base url', () => {
    const result = isZaiFile(makeFile({ provider: 'openai', baseUrl: 'https://api.openai.com/v1' }));
