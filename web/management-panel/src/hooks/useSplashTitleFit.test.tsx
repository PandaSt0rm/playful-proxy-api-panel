...
function makeElements(opts: {
  ...
  Object.defineProperty(container, 'clientWidth', { get: () => opts.widthLimit, configurable: true });
...
describe('useSplashTitleFit — binary-search fit result', () => {
  it('does nothing before the scheduled frame runs (no font size written yet)', () => {
    ...
    renderHook(() => useSplashTitleFit(refTo(container), refTo(title), 'key', { minFontSize: 10, maxFontSize: 100 }));
  ...
  it('writes the computed size to a custom CSS variable name', () => {
    const { container, title } = makeElements({ widthLimit: 9999, k: 0, variableName: '--custom-size' });
    renderHook(() =>
  ...
  it('shrinks the title when the parent height is the limiting dimension', () => {
    ...
    void parent;
    renderHook(() => useSplashTitleFit(refTo(container), refTo(title), 'key', { minFontSize: 1, maxFontSize: 20 }));
  ...
  it('re-runs the fit when the dependency key changes', () => {
    ...
    const { rerender } = renderHook(
      ({ depKey }: { depKey: string }) =>
        useSplashTitleFit(refTo(container), refTo(title), depKey, { minFontSize: 10, maxFontSize: 100 }),
...
describe('useSplashTitleFit — cleanup', () => {
  it('cancels the pending frame on unmount so no fit runs afterward', () => {
    ...
    const { unmount } = renderHook(() =>
      useSplashTitleFit(refTo(container), refTo(title), 'key', { minFontSize: 10, maxFontSize: 100 })
    );
