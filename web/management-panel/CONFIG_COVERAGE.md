...
| `proxy-url`, `passthrough-headers`, `disable-image-generation`, `enable-gemini-cli-endpoint`, `force-model-prefix`, `ws-auth`, `codex.identity-confuse` | Config visual editor; dashboard shows image generation and WebSocket auth | `/config.yaml` plus existing simple endpoints where available | Network/request behavior is visible without source editing. `codex.identity-confuse` is a typed toggle in the Network section. |
