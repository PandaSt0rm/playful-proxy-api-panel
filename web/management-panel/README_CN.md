...
用于管理与故障排查 **Playful Proxy API Panel (PPAP)** / CLIProxyAPI 兼容后端的单文件 Web UI（React + TypeScript），通过 **Management API** 完成配置、凭据与日志等管理操作。

[English](README.md)

**主项目**: https://github.com/daishuge/playful-proxy-api-panel

**主维护位置**: 此前端维护在主仓的 `web/management-panel` 目录中；旧的独立面板仓库会保留一段时间，方便用户过渡。
